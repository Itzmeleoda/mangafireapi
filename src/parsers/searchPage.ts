...
export const scrapeSearchResults = async (keyword: string, page: number = 1): Promise<ScrapedSearchResults> => {
  ...
  try {
    const content = await client.get(`/filter?keyword=${encodeURIComponent(keyword)}&page=${page}`);
    ...
    $('div.original.card-lg > div.unit').each((_, el) => {
      const r: SearchResult = { id: hrefToMangaId($(el).find('a.poster').attr('href')), title: $(el).find('div.info > a').text().trim() || null, poster: $(el).find('a.poster > div > img').attr('src')?.trim() || null, type: $(el).find('div.info > div > span.type').text().trim() || null, chapters: [] };
